...
format:
  ...
  pdf:
    ...
    include-in-header:
      text: |
        % wrap long code lines (e.g. the Homebrew install command) in the PDF
        \usepackage{fvextra}
...
install.packages(c("tidyverse", "arrow", "haven", "gtsummary",
                   "GGally", "mosaicData",
                   # downlit and xml2 turn every function name in a rendered
                   # document into a link to its help page, so you can click
                   # a function in the demos to read what it does
                   "downlit", "xml2"))
```
